...
interface Account {
  ...
  syncState: "none" | "syncing" | "ready" | "error";
  syncError?: string;
  lastSynced?: string;
  notice?: string;
}
...
export default function App({ mode, storeName, snapshot, briefing, alsoNoticed, account }: Props) {
  ...
  const products = useMemo(() => {
    ...
    return snapshot.products.map((p) => {
      ...
      return {
        ...p, units, vel,
        tracked: p.stock !== null,
        dts: p.stock === null || p.stock === 0 ? null : vel > 0 ? p.stock / vel : null,
  ...
  if (products.length === 0 && mode === "account") {
    return (
      <>
        <header className="top"><div className="top-in">
          <a href="/"><img className="wordmark" src="/logo-white.png" alt="Storecasted" /></a>
          <div className="top-right"><p className="store">{storeName}<span>{account?.email}</span></p>
            <form action="/auth/signout" method="post"><button className="btn btn-quiet-top">Sign out</button></form></div>
        </div></header>
        <main>
          {account && <ConnectBar account={account} />}
          <h1 className="hero">Nothing to read yet.</h1>
          <p className="sub">Your Square account connected, but it doesn&apos;t have any items with a fixed price yet. Add a few items and some sales in Square, then hit Sync now.</p>
        </main>
      </>
    );
  }
    ...
    <>
      <header className="top">
        <div className="top-in">
          ...
          <a href="/" aria-label="Storecasted home"><img className="wordmark" src="/logo-white.png" alt="Storecasted" /></a>
          {(mode === "demo" || account?.syncState !== "ready") && <span className="demo-tag">Demo store</span>}
          {mode === "demo" ? (
            <div className="top-right">
              <p className="store">{storeName}<span>Sample data · real engine</span></p>
              <a className="btn btn-top" href="/login?mode=signup">Sign up free</a>
            </div>
          ) : (
            <div className="top-right">
              <p className="store">{storeName}<span>{account?.email}</span></p>
              <form action="/auth/signout" method="post"><button className="btn btn-quiet-top">Sign out</button></form>
            </div>
          )}
        </div>
      </header>
      ...
      <main>
        {mode === "account" && account && <ConnectBar account={account} />}
        {tab === "today" && (
          <Today
...
/* ---------------- account bar ---------------- */
function ConnectBar({ account }: { account: Account }) {
  const router = useRouter();
  // While the first sync runs, check back every few seconds.
  useEffect(() => {
    if (account.syncState !== "syncing") return;
    const t = setInterval(() => router.refresh(), 6000);
    return () => clearInterval(t);
  }, [account.syncState, router]);

  const syncNow = (label: string) => (
    <form action="/api/square/sync" method="post"><button className="btn">{label}</button></form>
  );
  ...
  if (account.needsReconnect) {
    body = (
      <div className="connect">
        <div><p className="connect-h">Square needs reconnecting.</p>
          <p className="connect-s">Square stopped sharing some of your data. Reconnect to get your briefing back.</p></div>
        <a className="btn" href="/api/square/connect">Reconnect Square</a>
      </div>
    );
  } else if (account.syncState === "none") {
    body = (
      <div className="connect">
        <div><p className="connect-h">You&apos;re looking at a demo store.</p>
          <p className="connect-s">Connect Square and this becomes your shop. Storecasted only ever reads — it can&apos;t change anything in your account.</p></div>
        <a className="btn" href="/api/square/connect">Connect Square</a>
      </div>
    );
  } else if (account.syncState === "syncing") {
    body = (
      <div className="connect">
        <div><p className="connect-h">Pulling in your Square data…</p>
          <p className="connect-s">Your sales, stock and restock history. This page switches to your shop by itself when it&apos;s done — usually a minute or two.</p></div>
      </div>
    );
  } else if (account.syncState === "error") {
    body = (
      <div className="connect">
        <div><p className="connect-h">Square didn&apos;t finish syncing.</p>
          <p className="connect-s">{account.syncError ?? "Something went wrong reading your data."}</p></div>
        {syncNow("Try again")}
      </div>
    );
  } else {
    body = (
      <div className="synced">
        <span>Showing your Square data{account.lastSynced ? ` · last synced ${new Date(account.lastSynced).toLocaleString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}` : ""}</span>
        <form action="/api/square/sync" method="post"><button className="link-btn">Sync now</button></form>
      </div>
    );
  }
  ...
  return (
    <>
      {account.notice && <p className="banner" style={{ marginBottom: 16 }}>{account.notice}</p>}
      {body}
    </>
  );
}
...
/* ---------------- suggested order ---------------- */
export function OrderCallout({ facts }: { facts: Record<string, string | number> }) {
  const single = facts["order_units"];
  const count = Number(facts["order_count"] ?? 0);
  if (single === undefined && count === 0) return null;

  const cost = facts["order_cost"];
  if (single !== undefined) {
    return (
      <div className="order">
        <p className="order-l">Order now</p>
        <p className="order-v">Order {single}</p>
        <p className="order-s">
          Covers ~{facts["order_cover_days"]} days
          {facts["order_basis"] === "restock_cycle" ? ", as long as your last restock lasted" : ""}, plus {facts["order_buffer_pct"]}%
          {cost !== undefined ? ` · ${cost} at your cost` : ""}
        </p>
      </div>
    );
  }
  const rows: Array<[string, string | number]> = [];
  for (let i = 1; i <= count; i++) rows.push([String(facts[`order_item_${i}`]), facts[`order_units_${i}`] ?? 0]);
  return (
    <div className="order">
      <p className="order-l">Order to restock</p>
      {rows.map(([name, units]) => (
        <div className="order-row" key={name}><span>{name}</span><b>Order {units}</b></div>
      ))}
      <p className="order-s" style={{ marginTop: 10 }}>
        About a month&apos;s worth at the rate each sold before running out, plus {facts["order_buffer_pct"]}%
        {cost !== undefined ? ` · ${cost} at your cost` : " · some items have no cost in Square, so no total"}
      </p>
    </div>
  );
}
...
function Today({ today, hero, briefing, alsoNoticed, products, N, agg }: {
  ...
  products: Array<{ squareItemId: string; units: number[]; priceCents: number; unitCostCents: number | null; stock: number | null }>;
...
function Dashboard({ products, N, buckets, agg, orders, metricAt, dateAt, snapshot }: {
  ...
  for (const p of products) {
    if (p.stock === null) continue;           // doesn't track inventory
  ...
  return (
    <section>
      ...
      <dl className="strip" style={{ marginTop: 0 }}>
        <Cell t="Units on hand" v={num(onHand)} dl={{ cls: "flat", text: `across ${products.filter((p) => p.stock !== null).length} tracked items` }} />
...
function Products({ products, buckets, agg, snapshot }: {
  ...
  const rows = products.map((p) => {
    const g = agg([p], a, b);
    return {
      p, name: p.name as string, stock: p.stock as number | null, vel: p.vel as number,
      dts: p.stock === null ? 99999 : p.stock === 0 ? -1 : p.dts ?? 9999, recv: daysAgo(p.lastReceivedAt), ret: p.returns as number,
  ...
  const stateOf = (p: any) => {
    if (p.stock === null) return "var(--rule)";
        ...
        <table>
          ...
          <tbody>
            {rows.map((r) => {
              ...
              const runs = r.stock === null ? "Not tracked" : r.stock === 0 ? "Out of stock" : r.dts > 120 ? "Months away" : `${Math.round(r.dts)} days`;
              ...
              return [
                <tr className="row" key={r.name} onClick={() => setOpen(isOpen ? null : r.name)}>
                  ...
                  <td className="num">{r.stock ?? <span className="muted">—</span>}</td>
