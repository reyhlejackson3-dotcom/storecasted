import { DEFAULT_REORDER, centsToUsd, restockCycleDays, suggestedReorder } from "./metrics";

const BUFFER_PCT = Math.round(DEFAULT_REORDER.buffer * 100);
...
function outOfStockSignals(m: StoreMetrics): Signal[] {
  ...
  // One suggested order per item. Numbered keys keep facts flat, which the
  // number guard and the templates both rely on.
  const orders: Record<string, string | number> = {};
  let allCosted = true;
  let costTotal = 0;
  let n = 0;
  for (const p of worst) {
    const r = suggestedReorder(p);
    if (!r) continue;
    n++;
    orders[`order_item_${n}`] = p.name;
    orders[`order_units_${n}`] = r.units;
    orders[`order_rate_${n}`] = Number(p.velocityPrior.toFixed(1));
    if (r.costCents === null) allCosted = false;
    else costTotal += r.costCents;
  }
  ...
  return [
    {
      ...
      facts: {
        ...
        order_count: n,
        order_buffer_pct: BUFFER_PCT,
        ...orders,
        ...(n > 0 && allCosted ? { order_cost: centsToUsd(costTotal) } : {}),
      },
    },
  ];
}
...
function stockoutSignals(m: StoreMetrics, cfg: RuleConfig): Signal[] {
  return m.products
    ...
    .map((p): Signal => {
      ...
      const order = suggestedReorder(p);
      return {
        ...
        facts: {
          item: p.name,
          stock: p.stock ?? 0, // only reached for tracked items
          ...
          ...(order
            ? {
                ...
                order_cover_days: order.coverDays,
                order_basis: order.basis,
                order_buffer_pct: BUFFER_PCT,
                ...(order.costCents !== null ? { order_cost: centsToUsd(order.costCents) } : {}),
              }
            : {}),
        },
      };
